...
💡 Exemplo simples
Imagine ensinar um sistema a reconhecer gatos:
Você mostra milhares de fotos de gatos e não-gatos.
O modelo aprende características (orelhas, olhos, formato).
Depois, consegue identificar novos gatos sozinho.
🔧 Como funciona (bem resumido)
Você fornece dados (ex: imagens, textos, números).
O algoritmo analisa esses dados.
Ele encontra padrões.
Usa esses padrões para prever ou decidir algo novo.
📊 Principais tipos de machine learning
Supervisionado: aprende com dados já rotulados (ex: “isso é gato”, “isso não é”).
Não supervisionado: encontra padrões sem rótulos (ex: agrupar clientes por comportamento).
Por reforço: aprende por tentativa e erro, recebendo recompensas (ex: jogos, robótica).
🧠 Onde isso é usado?
